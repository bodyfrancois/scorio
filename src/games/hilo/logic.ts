...
import { hiloConfig } from './config';

export const hiloEngine: GameEngine = {
  config: hiloConfig,
  ...
  checkEndGame(scores, players, scoreLimit?, roundLimit?) {
    ...
    const effectiveRoundLimit = roundLimit ?? hiloConfig.roundLimit;
    if (effectiveRoundLimit != null) {
      if (scores[0].length < effectiveRoundLimit) return { hasEnded: false };
      return { hasEnded: true, ranking: sortRankingAscending(players, totals) };
    }

    const limit = scoreLimit ?? hiloConfig.scoreLimit ?? 500;
