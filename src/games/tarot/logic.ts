...
import { tarotConfig } from './config';

export const tarotEngine: GameEngine = {
  config: tarotConfig,
  ...
  checkEndGame(scores, players, scoreLimit?, roundLimit?) {
    ...
    const effectiveRoundLimit = roundLimit ?? tarotConfig.roundLimit;
    ...
    const limit = scoreLimit ?? tarotConfig.scoreLimit ?? 500;
