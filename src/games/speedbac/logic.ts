...
import { speedbacConfig } from './config';

export const speedbacEngine: GameEngine = {
  config: speedbacConfig,
  ...
  checkEndGame(scores, players, scoreLimit?, roundLimit?) {
    ...
    const effectiveRoundLimit = roundLimit ?? speedbacConfig.roundLimit;
    ...
    const limit = scoreLimit ?? speedbacConfig.scoreLimit ?? 500;
